    ...
    <header className="sticky top-0 z-50 border-b border-ink/10 bg-white/80 backdrop-blur dark:border-white/10 dark:bg-black/80">
      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 text-ink dark:text-white">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/profile" className="hidden sm:block"><UserRound size={18} /></Link>
      ...
      {open && <nav className="flex flex-col gap-5 border-t border-ink/10 bg-white px-6 py-6 text-ink dark:border-white/10 dark:bg-black dark:text-white md:hidden">{links.map(([label, href]) => <Link onClick={() => setOpen(false)} key={label} href={href} className="text-sm font-semibold uppercase tracking-widest">{label}</Link>)}</nav>}
    </header>
  );
}