...
export function AuthShell({ eyebrow, title, copy, highlights, media, titleClassName, children }: AuthShellProps) {
		...
		<div className="text-ink dark:text-white">
			...
			<div className="mt-8 grid gap-4 sm:grid-cols-2">
				<div className="relative overflow-hidden rounded-[2rem] shadow-[0_22px_50px_rgba(20,18,15,.12)]">
					<img src="https://images.unsplash.com/photo-1541643600914-78b084683601?auto=format&fit=crop&w=1000&q=85" alt="Elegant perfume bottles and warm lighting" className="h-64 w-full object-cover" />
				...
				<div className="luxury-panel rounded-[1.75rem] p-5">
					<p className="eyebrow">Refined access</p>
					<p className="mt-3 text-sm leading-7 text-ink/70 dark:text-white/70">A minimal, polished flow that complements the sign in form without extra visual clutter.</p>
				</div>
			</div>
	...
	return (
		<section className="relative overflow-hidden px-6 py-10 sm:px-8 lg:px-10">
			...
			<div className="mx-auto grid max-w-6xl gap-10 lg:min-h-[calc(100vh-8rem)] lg:grid-cols-2 lg:items-stretch">
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38, ease: 'easeOut' }} className="flex h-full">
					<div className="flex h-full w-full">{media || defaultMedia}</div>
				</motion.div>

				<motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.42, delay: 0.06, ease: 'easeOut' }} className="flex h-full">
					<div className="luxury-panel h-full w-full rounded-[2rem] p-6 shadow-[0_30px_90px_rgba(20,18,15,.12)] sm:p-8 lg:p-10">
						{children}
					</div>
				</motion.div>
			</div>
